...
import type { CreateStaffInput, StaffResponse, UpdateStaffInput } from "./types";

async function listStaffs() {
    const response = await http.get<BaseApiResponse<StaffResponse[]>>("/api/admin/staffs");
    return response.data;
}

async function getStaff(id: string) {
    const response = await http.get<BaseApiResponse<StaffResponse>>(`/api/admin/staffs/${id}`);
    return response.data;
}

async function createStaff(input: CreateStaffInput) {
    const response = await http.post<BaseApiResponse<StaffResponse>>("/api/admin/staffs", input);
    return response.data;
}
